...
def eval_poly(z: complex, coeffs: torch.Tensor) -> complex:
    ...
    if not isinstance(z, torch.Tensor):
        z = torch.tensor(z, dtype=torch.complex128)

    # Start with highest degree coefficient
    result = coeffs[0].expand_as(z)

    # Use Horner's method: p(x) = a_n + x(a_{n-1} + x(a_{n-2} + ...))
    for c in coeffs[1:]:
        result = result * z + c
...
def hankel_brute(p_coeffs: torch.Tensor, T: int, beta: float, Nxy: int = 601) -> torch.Tensor:
    """
...
def accurate_zt_matrix(p_coeffs: torch.Tensor, T: int, beta: float, num_points: int = 601) -> torch.Tensor:
...
def accurate_zt_matrix_vectorized(p_coeffs: torch.Tensor, T: int, beta: float, num_points: int = 601) -> torch.Tensor:
    ...
    # Create complex grid and evaluate polynomial
    alpha = torch.complex(X[mask], Y[mask])  # More efficient than X + 1j * Y
    val_p = eval_poly(alpha, p_coeffs)
    val_pc = eval_poly(alpha.conj(), p_coeffs)
    fac = val_p * val_pc

    # Create powers matrix
    i_indices = torch.arange(T, dtype=torch.float64).to(torch.complex128)
    powers_alpha = alpha.unsqueeze(1) ** i_indices  # Shape: (N_valid_points, T)
    powers_alpha_conj = alpha.conj().unsqueeze(1) ** i_indices

    # Compute final matrix using batch operations
